...
class Snefuru_Admin {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_update_box_ordering_active', array($this, 'handle_update_box_ordering_active'));
...
3. Updates the widget's content if there's a match</pre>
    ...
    public function handle_create_box_order_config() {
        ...
        try {
            ...
            if (!wp_verify_nonce($_POST['nonce'], 'box_order_nonce')) {
                $debug_info['error_stage'] = 'nonce_verification';
                $debug_info['nonce_provided'] = $_POST['nonce'];
                wp_send_json_error([
            ...
            // Check permissions
            if (!current_user_can('manage_options')) {
                $debug_info['error_stage'] = 'permissions_check';
                $debug_info['user_capabilities'] = array_keys(wp_get_current_user()->allcaps);
                wp_send_json_error([
            ...
            $post_id = intval($_POST['post_id']);
            if (!$post_id) {
                $debug_info['error_stage'] = 'post_id_validation';
                $debug_info['post_id_raw'] = $_POST['post_id'];
                wp_send_json_error([
            ...
            if (!$post) {
                $debug_info['error_stage'] = 'post_exists_check';
            ...
            $debug_info['table_info'] = [
                'table_name' => $box_orders_table,
                'table_exists' => $table_exists
            ];
            
            if (!$table_exists) {
                $debug_info['error_stage'] = 'table_check';
                $debug_info['sql_query'] = "SHOW TABLES LIKE '$box_orders_table'";
                wp_send_json_error([
                    'message' => 'Box orders table does not exist',
                    'debug_info' => $debug_info
                ]);
                return;
            }
            ...
            $debug_info['table_structure'] = $wpdb->get_results("DESCRIBE $box_orders_table", ARRAY_A);
            
            // Check if config already exists
            $check_query = $wpdb->prepare("SELECT * FROM {$box_orders_table} WHERE rel_post_id = %d", $post_id);
            $debug_info['check_query'] = $check_query;
            
            $existing = $wpdb->get_row($check_query);
            $debug_info['existing_config'] = $existing;
            
            if ($existing) {
                $debug_info['error_stage'] = 'duplicate_check';
                wp_send_json_error([
                    'message' => 'Box order config already exists for this post',
                    'debug_info' => $debug_info
                ]);
                return;
            }
            ...
            $json_data = json_encode($default_boxes);
            $debug_info['default_boxes'] = $default_boxes;
            ...
            $insert_data = [
                'rel_post_id' => $post_id,
                'is_active' => 1,
                'box_order_json' => $json_data
            ];
            $debug_info['insert_data'] = $insert_data;
            
            // Insert new config
            $result = $wpdb->insert($box_orders_table, $insert_data);
            ...
            if ($result === false) {
                $debug_info['error_stage'] = 'database_insert';
                wp_send_json_error([
                    'message' => 'Failed to create box order config',
                    'detailed_error' => $wpdb->last_error ?: 'Unknown database error',
                    'debug_info' => $debug_info
                ]);
                return;
            }
            ...
            wp_send_json_success([
                'message' => 'Box order config created successfully',
                'insert_id' => $wpdb->insert_id,
                'debug_info' => $debug_info
            ]);
            
        } catch (Exception $e) {
            $debug_info['error_stage'] = 'exception_caught';
    ...
    public function handle_save_box_order_config() {
        ...
        try {
            ...
            $debug_info['processed_data'] = [
                ...
                'json_preview' => substr($box_order_json, 0, 200) . (strlen($box_order_json) > 200 ? '...' : '')
            ];
            ...
            $debug_info['json_validation'] = [
                'error_code' => $json_error,
                'error_message' => json_last_error_msg(),
                'decoded_data' => $box_order
            ];
            
            if ($json_error !== JSON_ERROR_NONE) {
                $debug_info['error_stage'] = 'json_validation';
                wp_send_json_error([
            ...
            $check_query = $wpdb->prepare("SELECT item_id FROM {$box_orders_table} WHERE rel_post_id = %d", $post_id);
            $existing = $wpdb->get_row($check_query);
            $debug_info['existing_check'] = [
                'query' => $check_query,
            ...
            if ($result === false) {
                $debug_info['error_stage'] = 'database_operation';
                wp_send_json_error([
                    'message' => 'Failed to save box order config',
                    'detailed_error' => $wpdb->last_error ?: 'Unknown database error',
                    'debug_info' => $debug_info
                ]);
                return;
            ...
            wp_send_json_success([
                'message' => 'Box order saved successfully',
                'debug_info' => $debug_info
            ]);
    ...
    public function handle_delete_box_order_config() {
        ...
        try {
            ...
            $post_id = intval($_POST['post_id']);
            $debug_info['processed_data'] = ['post_id' => $post_id];
            ...
            $debug_info['existing_record'] = [
                'query' => $check_query,
                'record' => $existing_record,
                'record_exists' => !empty($existing_record)
            ];
            ...
            if ($result === false) {
                $debug_info['error_stage'] = 'database_operation';
                wp_send_json_error([
                    'message' => 'Failed to delete box order config',
                    'detailed_error' => $wpdb->last_error ?: 'Unknown database error',
                    'debug_info' => $debug_info
                ]);
                return;
            ...
            wp_send_json_success([
                'message' => 'Box order config deleted successfully',
                'debug_info' => $debug_info
            ]);
    ...
    /**
     * AJAX handler for updating box ordering active status
     */
    public function handle_update_box_ordering_active() {
        // Initialize debug info
        $debug_info = [
            'timestamp' => current_time('mysql'),
        ...
        try {
            ...
            $debug_info['existing_record'] = [
                'record_exists' => !empty($existing_record),
                'current_is_active' => $existing_record ? $existing_record->is_active : null
            ];
            ...
            if ($existing_record) {
                ...
                // Create new record with default box order
                $default_box_order = [
                    'batman_hero_box' => 1,
                    'chen_cards_box' => 2,
                    'plain_post_content' => 3,
                ...
                $result = $wpdb->insert(
                    ...
                    [
                        'rel_post_id' => $post_id,
                        'is_active' => $is_active,
                        'box_order_json' => json_encode($default_box_order),
                        'created_at' => current_time('mysql'),
                        'updated_at' => current_time('mysql')
                    ]
                );
                $debug_info['operation'] = 'insert';
                $debug_info['default_box_order'] = $default_box_order;
            }
            ...
            if ($result === false) {
                $debug_info['error_stage'] = 'database_operation';
                wp_send_json_error([
                    'message' => 'Failed to update box ordering status',
                    'detailed_error' => $wpdb->last_error ?: 'Unknown database error',
                    'debug_info' => $debug_info
                ]);
                return;
            ...
            wp_send_json_success([
                'message' => "Box ordering {$status_text} successfully",
                'debug_info' => $debug_info
            ]);
