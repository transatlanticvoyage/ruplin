...
class Snefuru_Cobalt {
    ...
    public function process_cobalt_submission($post_id, $cobalt_content, $auto_update_title = true) {
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] process_cobalt_submission called with post_id: ' . $post_id . PHP_EOL, FILE_APPEND);
        
        // Special reset command to restore backup data
        if (trim($cobalt_content) === 'RESET_ELEMENTOR_DATA') {
        ...
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Elementor data length: ' . strlen($elementor_data) . PHP_EOL, FILE_APPEND);
        ...
        if (!$elements || !is_array($elements)) {
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] JSON decode failed, error: ' . json_last_error_msg() . PHP_EOL, FILE_APPEND);
        ...
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Parsed mappings: ' . print_r($content_mappings, true) . PHP_EOL, FILE_APPEND);
        ...
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Total updates made: ' . $updates_made . PHP_EOL, FILE_APPEND);
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Final widget counter: ' . $widget_counter . PHP_EOL, FILE_APPEND);
        ...
        if ($updates_made > 0) {
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Elements array corrupted during processing: ' . json_last_error_msg() . PHP_EOL, FILE_APPEND);
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] JSON encode error: ' . json_last_error_msg() . PHP_EOL, FILE_APPEND);
                return array('success' => false, 'message' => 'Failed to encode updated data: ' . json_last_error_msg());
            }
            
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] JSON encoded successfully, length: ' . strlen($json_data) . PHP_EOL, FILE_APPEND);
            
            // Test decode without wp_slash first
            $test_decode_clean = json_decode($json_data, true);
            if (json_last_error() !== JSON_ERROR_NONE) {
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Clean JSON decode failed: ' . json_last_error_msg() . PHP_EOL, FILE_APPEND);
                return array('success' => false, 'message' => 'Clean JSON decode failed: ' . json_last_error_msg());
            }
            
            // Apply wp_slash
            $updated_data = wp_slash($json_data);
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Applied wp_slash, length: ' . strlen($updated_data) . PHP_EOL, FILE_APPEND);
            
            // Test that our data can be decoded after wp_slash
            $test_decode = json_decode($updated_data, true);
            if (json_last_error() !== JSON_ERROR_NONE) {
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Post wp_slash decode failed: ' . json_last_error_msg() . PHP_EOL, FILE_APPEND);
                
                // Try without wp_slash
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Trying save without wp_slash...' . PHP_EOL, FILE_APPEND);
                $updated_data = $json_data; // Use original without wp_slash
            }
            
            // Try different approaches to save the data
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Trying update_post_meta...' . PHP_EOL, FILE_APPEND);
            $save_result = update_post_meta($post_id, '_elementor_data', $updated_data);
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] update_post_meta result: ' . ($save_result ? 'TRUE' : 'FALSE') . PHP_EOL, FILE_APPEND);
            
            if (!$save_result) {
                // Try direct database query instead of delete/add approach
                global $wpdb;
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] update_post_meta failed, trying direct database update...' . PHP_EOL, FILE_APPEND);
                $db_result = $wpdb->update(
                    $wpdb->postmeta,
                    array('meta_value' => $updated_data),
                    array('post_id' => $post_id, 'meta_key' => '_elementor_data')
                );
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Database update result: ' . $db_result . ' (0=no change, >0=success, false=error)' . PHP_EOL, FILE_APPEND);
                
                if ($db_result === false) {
                    // Last resort: delete old and add new, but do it carefully
                    file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Direct update failed, trying delete-specific and add...' . PHP_EOL, FILE_APPEND);
                    ...
                    if ($current_meta) {
                        ...
                        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Deleted specific meta entry, result: ' . $delete_result . PHP_EOL, FILE_APPEND);
                        
                        // Add the new data
                        $add_result = add_post_meta($post_id, '_elementor_data', $updated_data, true);
                        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] add_post_meta result: ' . ($add_result ? 'TRUE' : 'FALSE') . PHP_EOL, FILE_APPEND);
                    }
                }
            }
            ...
            error_log('Cobalt Debug - Cleared WordPress object cache for post');
            ...
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Found ' . count($meta_entries) . ' _elementor_data entries' . PHP_EOL, FILE_APPEND);
            
            foreach ($meta_entries as $i => $entry) {
                $entry_length = strlen($entry->meta_value);
                $contains_drain = stripos($entry->meta_value, 'drain') !== false;
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Entry ' . ($i+1) . ' (ID: ' . $entry->meta_id . '): Length=' . $entry_length . ', Contains drain=' . ($contains_drain ? 'YES' : 'NO') . PHP_EOL, FILE_APPEND);
            }
            
            // Verify the save by re-reading the data using get_post_meta
            $saved_data = get_post_meta($post_id, '_elementor_data', true);
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] get_post_meta returned data length: ' . strlen($saved_data) . PHP_EOL, FILE_APPEND);
            ...
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] get_post_meta result contains "drain": ' . ($contains_drain ? 'YES' : 'NO') . PHP_EOL, FILE_APPEND);
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] get_post_meta result contains "Amazing Plumbing People": ' . ($contains_amazing ? 'YES' : 'NO') . PHP_EOL, FILE_APPEND);
            
            // Test JSON validity
            $test_decode = json_decode($saved_data, true);
            $json_valid = (json_last_error() === JSON_ERROR_NONE);
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] JSON is valid: ' . ($json_valid ? 'YES' : 'NO') . PHP_EOL, FILE_APPEND);
            if (!$json_valid) {
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] JSON error: ' . json_last_error_msg() . PHP_EOL, FILE_APPEND);
            }
            ...
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Contains "Great drain cleaning": ' . ($contains_drain_ci ? 'YES' : 'NO') . PHP_EOL, FILE_APPEND);
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Contains "Amazing Plumbing": ' . ($contains_amazing_ci ? 'YES' : 'NO') . PHP_EOL, FILE_APPEND);
            
            // Save a sample of the data to see what's actually in there
            $sample = substr($saved_data, 0, 500);
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Data sample (first 500 chars): ' . $sample . PHP_EOL, FILE_APPEND);
            ...
            error_log('Cobalt Debug - Verification: Widget1 content found: ' . ($found_widget1_content ? 'YES' : 'NO'));
            error_log('Cobalt Debug - Verification: Widget2 content found: ' . ($found_widget2_content ? 'YES' : 'NO'));
    ...
    private function process_elementor_element_cobalt(&$element, $content_mappings, &$widget_counter, &$updates_made) {
        ...
        if (!empty($element['widgetType'])) {
            ...
            if ($widget_has_content) {
                ...
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Processing widget ' . $widget_counter . ' (type: ' . $element['widgetType'] . ')' . PHP_EOL, FILE_APPEND);
                
                // Check if we have content for this widget
                if (isset($content_mappings[$widget_key])) {
                    $mapping = $content_mappings[$widget_key];
                    file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Found mapping for ' . $widget_key . ': ' . print_r($mapping, true) . PHP_EOL, FILE_APPEND);
                    
                    // Handle multi-item widgets (with items)
                    if (!empty($mapping['items'])) {
                        $updated = $this->update_multi_item_widget($element, $mapping['items']);
                        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Multi-item widget update result: ' . ($updated ? 'success' : 'failed') . PHP_EOL, FILE_APPEND);
                    }
                    // Handle single content widgets
                    elseif (!empty($mapping['content'])) {
                        $updated = $this->update_single_widget($element, $mapping['content']);
                        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Single widget update result: ' . ($updated ? 'success' : 'failed') . PHP_EOL, FILE_APPEND);
                    }
                    ...
                    file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] No mapping found for ' . $widget_key . PHP_EOL, FILE_APPEND);
                }
                
                $widget_counter++;
            } else {
                file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Skipping widget ' . $element['widgetType'] . ' (no extractable content)' . PHP_EOL, FILE_APPEND);
            }
        }
    ...
    private function update_single_widget(&$element, $new_content) {
        if (empty($element['widgetType']) || empty($element['settings'])) {
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] update_single_widget: Missing widgetType or settings' . PHP_EOL, FILE_APPEND);
            return false;
        }
        ...
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] update_single_widget: Processing ' . $widget_type . ' with content: ' . $new_content . PHP_EOL, FILE_APPEND);
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Available settings: ' . print_r(array_keys($element['settings']), true) . PHP_EOL, FILE_APPEND);
        
        switch ($widget_type) {
            case 'heading':
                if (isset($element['settings']['title'])) {
                    $old_title = $element['settings']['title'];
                    file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Updating heading title from "' . $old_title . '" to "' . $new_content . '"' . PHP_EOL, FILE_APPEND);
                    $element['settings']['title'] = $new_content;
                    file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] After update, element title is now: "' . $element['settings']['title'] . '"' . PHP_EOL, FILE_APPEND);
                    $updated = true;
                } else {
                    file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Heading widget has no title setting' . PHP_EOL, FILE_APPEND);
                }
                break;
            ...
            case 'text-editor':
                if (isset($element['settings']['editor'])) {
                    ...
                    file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Updating text-editor from "' . substr($old_content, 0, 50) . '..." to "' . $new_content . '"' . PHP_EOL, FILE_APPEND);
                    $element['settings']['editor'] = $new_content;
                    file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] After update, editor content is now: "' . $element['settings']['editor'] . '"' . PHP_EOL, FILE_APPEND);
                    $updated = true;
                } else {
                    file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Text-editor widget has no editor setting' . PHP_EOL, FILE_APPEND);
                }
                break;
        ...
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] update_single_widget: Returning ' . ($updated ? 'true' : 'false') . PHP_EOL, FILE_APPEND);
        return $updated;
    }
        ...
        if (!empty($element['widgetType'])) {
            ...
            if ($widget_has_content) {
                if ($widget_counter == 1 && !empty($element['settings']['title']) && 
                    ...
                    $found_widget1 = true;
                    error_log('Cobalt Debug - Found widget1 content: ' . $element['settings']['title']);
                }
                
                if ($widget_counter == 2 && !empty($element['settings']['editor']) && 
                    $element['settings']['editor'] === 'Great drain cleaning services') {
                    $found_widget2 = true;
                    error_log('Cobalt Debug - Found widget2 content: ' . $element['settings']['editor']);
                }
    ...
    public function ajax_cobalt_inject_content() {
        ...
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] COBALT AJAX HANDLER CALLED - START' . PHP_EOL, FILE_APPEND);
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] POST data: ' . print_r($_POST, true) . PHP_EOL, FILE_APPEND);
        
        // Verify nonce for security
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'hurricane_nonce')) {
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Security check failed' . PHP_EOL, FILE_APPEND);
        ...
        // Process the actual cobalt submission instead of just returning test message
        $result = $this->process_cobalt_submission($post_id, $content, $auto_update_title);
        
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Process result: ' . print_r($result, true) . PHP_EOL, FILE_APPEND);
    ...
    private function create_elementor_backup($post_id, $elementor_data) {
        ...
        file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Created backup: ' . $backup_key . PHP_EOL, FILE_APPEND);
    }
    ...
    private function restore_elementor_backup($post_id) {
        ...
        if ($restore_result !== false) {
            file_put_contents(WP_CONTENT_DIR . '/cobalt-debug.log', '[' . date('Y-m-d H:i:s') . '] Restored from backup: ' . $backup_meta->meta_key . PHP_EOL, FILE_APPEND);
